...
export default function LessonPanel({ course, lesson, currentUser, onComplete }) {
  ...
  if (lesson.type === 'quiz') {
    return (
      <div className="bg-white border-2 border-line rounded-[18px] p-7 mt-6">
  ...
  return (
    <div className="bg-white border-2 border-line rounded-[18px] p-7 mt-6">
      ...
      <div className="bg-[#FFF3D6] text-[#8A5B00] rounded-xl px-4 py-3.5 mb-4">💡 Fun fact: {lesson.fact}</div>
      <button
      ...
      {!currentUser && (
        <p className="text-[.82rem] text-ink-soft mt-3">Log in to save your progress.</p>
      )}
    </div>
  )
}
