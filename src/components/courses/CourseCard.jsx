...
export default function CourseCard({
  ...
  return (
    <div
      onClick={() => !comingSoon && navigate(`/courses/${course.id}`)}
      style={minWidth ? { minWidth } : undefined}
      className={`bg-white dark:bg-white/5 rounded-2xl border-2 h-[272px] border-line dark:border-white/10 overflow-hidden transition-all flex flex-col ${
        comingSoon ? "cursor-default opacity-80" : "cursor-pointer hover:scale-[0.98]"
      }`}
    >
      <div className="relative shrink-0">
        <CourseCover
          course={course}
          className={`h-28 ${comingSoon ? "grayscale" : ""}`}
          iconSize={52}
        />
        {comingSoon && (
          <div className="absolute top-0 inset-x-0 bg-sun text-ink text-center text-[.68rem] font-bold uppercase tracking-wide py-1">
            Coming soon
          </div>
        )}
      </div>
      <div className="p-5 flex-1 flex flex-col">
        <h3 className="text-[1.5rem] font-bold mb-1.5 flex-1">
          {course.title}
        </h3>
