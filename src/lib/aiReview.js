...
// Calls our own proxy server (server/index.js) instead of the AI provider
// directly — Gemini's API is meant to be called server-side with a
...
export async function analyzeResumeWithAI({ resumeText, targetRole }) {
  let res;
  try {
    res = await fetch(`${API_BASE}/api/analyze-cv`, {
      method: "POST",
