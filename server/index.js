...
function makeCrudRoutes(
  ...
  app.post(basePath, async (req, res) => {
    ...
    if (existing)
      return res
        .status(409)
        .json({ error: `Already exists: ${item[idField]}` });
...
async function start() {
  ...
  makeCrudRoutes("/api/paths", pathsCollection, {
    idField: "id",
    requiredFields: ["id", "label"],
  });
  makeCrudRoutes("/api/users", usersCollection, {
    idField: "username",
  ...
  app.patch("/api/courses/:courseId/lessons/:lessonId", async (req, res) => {
    ...
    const setDoc = Object.fromEntries(
      Object.entries(patch).map(([k, v]) => [`lessons.$.${k}`, v]),
    );
...
async function callGemini({
  ...
  if (!geminiRes.ok) {
    ...
    const message =
      body?.error?.message || `Gemini request failed (${geminiRes.status}).`;
...
app.post("/api/project-ideas", async (req, res) => {
  ...
  try {
    ...
    if (!ideas.length) {
      return res
        .status(502)
        .json({ error: "Gemini did not return any usable project ideas." });
    }
...
app.post("/api/learning-path", async (req, res) => {
  ...
  try {
    ...
    if (!steps.length) {
      return res
        .status(502)
        .json({ error: "Gemini did not return a usable learning path." });
    }
